...
type ColumnType = ColumnDef<MachineInterface, unknown>;
...
export function MachineListPage(props: MachineListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MachineInterface>>(
    () => `/machines?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/machines/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
